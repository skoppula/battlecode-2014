			...
			try{
				...
				if(rc.getType()==RobotType.HQ){//if I'm a headquarters
					...
					if (pastrs==0) {
						...
						rc.broadcast(444, 0);
						MapLocation goal = mypstrs[pastrs-1];
						System.out.println("THE HQ FOUND THE FOLLOWING GOAL : " + goal.x + "and" + goal.y);
						findgoal(rc, goal);
					}
					...
					System.out.println(goal.x + " and " + goal.y + "and numpasturs is" + num);
					...
					if (loc.distanceSquaredTo(goal)<5 ){
					...
					//Randomly set up pastrs where there are a ton of cows
					boolean a = rc.senseCowsAtLocation(loc.add(-1,1)) > 2000;
					boolean b = rc.senseCowsAtLocation(loc.add(1,1)) > 2000;
					boolean c = rc.senseCowsAtLocation(loc.add(-1,-1)) > 2000;
					boolean d = rc.senseCowsAtLocation(loc.add(1,-1)) > 2000;
					boolean e = true;
//					for (MapLocation i:pstrs ) {
//						if (loc.distanceSquaredTo(i)<2) {
//							e = false;
//						}
//					}
					if ((a||b||c||d) &&e&& rc.isActive()) {
						rc.construct(RobotType.PASTR);
					}
	...
	private static void findgoal(RobotController rc, MapLocation initialLoc) throws GameActionException {
		...
		int x_left = farthest(loc, Direction.WEST);
		int x_right = farthest(loc, Direction.EAST);
		int y_up = farthest(loc, Direction.NORTH);
		int y_down = farthest(loc, Direction.SOUTH);
		...
		if (next==x_left) {
			goal = loc.add(-x_left, 0);
			rc.broadcast(10, 7);
		} else if (next==x_right) {
			goal = loc.add(x_right, 0);
			rc.broadcast(10, 7);
		} else if (next==y_up) {
			goal = loc.add(0, -y_up);
			rc.broadcast(10, 1);
		} else if (next==y_down) {
			goal = loc.add(0, y_down);
			rc.broadcast(10, 1);
		}
	...
	private static int farthest(MapLocation loc, Direction d) {
		// TODO Auto-generated method stub
